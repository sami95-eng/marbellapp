// Direct Unsplash CDN URLs — no API key required.
// Règle : une URL unique par venue, zéro doublon dans tout le projet.

export const VENUE_IMAGES: Record<string, string> = {
  // ── Beach Clubs ──────────────────────────────────────────────────
  "ocean-club":
    "https://oceanclub.es/app/uploads/2025/06/home-3.png",
  "nikki-beach":
    "https://i0.wp.com/nikkibeach.com/marbella/wp-content/uploads/sites/6/2024/10/Hero_Marbella.jpg?ssl=1&w=2500&quality=85",
  "playa-padre":
    "https://images.unsplash.com/photo-1526922289011-a875fbd2fb0d?auto=format&fit=crop&w=800&q=80",
  "opium-beach":
    "https://images.unsplash.com/photo-1748509865532-bae58b4bd0ce?auto=format&fit=crop&w=800&q=80",
  "amare-beach":
    "https://images.unsplash.com/photo-1499793983690-e29da59ef1c2?auto=format&fit=crop&w=800&q=80",
  "amare-beach-club":
    "https://images.unsplash.com/photo-1499793983690-e29da59ef1c2?auto=format&fit=crop&w=800&q=80",
  "la-sala-sea":
    "https://images.unsplash.com/photo-1756680967373-c3205a8a8b31?auto=format&fit=crop&w=800&q=80",
  "puente-romano-beach-club":
    "https://images.unsplash.com/photo-1560472354-b33ff0c44a43?auto=format&fit=crop&w=800&q=80",
  "don-carlos-beach":
    "https://images.unsplash.com/photo-1548250531-94d59f8d5e71?auto=format&fit=crop&w=800&q=80",
  "marbella-club-beach":
    "https://images.unsplash.com/photo-1506929562872-bb421503ef21?auto=format&fit=crop&w=800&q=80",
  "starlite-beach":
    "https://images.unsplash.com/photo-1537640538966-79f369143f8f?auto=format&fit=crop&w=800&q=80",
  "salduna-beach":
    "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=800&q=80",
  "trocadero-arena":
    "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?auto=format&fit=crop&w=800&q=80",
  "bibo-beach-house":
    "https://images.unsplash.com/photo-1610641818989-c2051b5e2cfd?auto=format&fit=crop&w=800&q=80",

  // ── Fine Dining ───────────────────────────────────────────────────
  "lena-restaurant":
    "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=800&q=80",
  "lena":
    "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=800&q=80",
  "bibo-marbella":
    "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=800&q=80",
  "la-sala-marbella":
    "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?auto=format&fit=crop&w=800&q=80",
  "mosh-restaurant":
    "https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=800&q=80",
  "nobu-marbella":
    "https://images.unsplash.com/photo-1579871494447-9811cf80d66c?auto=format&fit=crop&w=800&q=80",
  "ta-kumi":
    "https://images.unsplash.com/photo-1482049016688-2d3e1b311543?auto=format&fit=crop&w=800&q=80",
  "sea-grill-nobu":
    "https://images.unsplash.com/photo-1543353071-873f17a7a088?auto=format&fit=crop&w=800&q=80",
  "el-puente-restaurante":
    "https://images.unsplash.com/photo-1559339352-11d035aa65de?auto=format&fit=crop&w=800&q=80",
  "puente-romano":
    "https://images.unsplash.com/photo-1589642073293-d0d511afb66e?auto=format&fit=crop&w=800&q=80",
  "lobito-de-mar":
    "https://images.unsplash.com/photo-1551024709-8f23befc6f87?auto=format&fit=crop&w=800&q=80",
  "calma":
    "https://images.unsplash.com/photo-1473093295043-cdd812d0e601?auto=format&fit=crop&w=800&q=80",
  "skina-restaurant":
    "https://images.unsplash.com/photo-1424847651672-bf20a4b0982b?auto=format&fit=crop&w=800&q=80",
  "skina":
    "https://images.unsplash.com/photo-1424847651672-bf20a4b0982b?auto=format&fit=crop&w=800&q=80",
  "messina-restaurant":
    "https://images.unsplash.com/photo-1504754524776-8f4f37790ca0?auto=format&fit=crop&w=800&q=80",
  "messina":
    "https://images.unsplash.com/photo-1504754524776-8f4f37790ca0?auto=format&fit=crop&w=800&q=80",
  "el-lago":
    "https://images.unsplash.com/photo-1476224203421-9ac39bcb3327?auto=format&fit=crop&w=800&q=80",
  "tragabuches-marbella":
    "https://images.unsplash.com/photo-1466978913421-dad2ebd01d17?auto=format&fit=crop&w=800&q=80",

  // ── Spa & Wellness ────────────────────────────────────────────────
  "six-senses-spa":
    "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?auto=format&fit=crop&w=800&q=80",
  "six-senses-spa-puente-romano":
    "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?auto=format&fit=crop&w=800&q=80",
  "finca-cortesin-spa":
    "https://images.unsplash.com/photo-1540555700478-4be290a0d474?auto=format&fit=crop&w=800&q=80",
  "amare-spa":
    "https://images.unsplash.com/photo-1515377905703-c4788e51af15?auto=format&fit=crop&w=800&q=80",
  "amare-spa-marbella":
    "https://images.unsplash.com/photo-1515377905703-c4788e51af15?auto=format&fit=crop&w=800&q=80",
  "puente-romano-spa":
    "https://images.unsplash.com/photo-1506126613408-eca07ce68773?auto=format&fit=crop&w=800&q=80",
  "spa-marbella-club":
    "https://images.unsplash.com/photo-1506126613408-eca07ce68773?auto=format&fit=crop&w=800&q=80",

  // ── Nightlife ─────────────────────────────────────────────────────
  "olivia-valere":
    "https://images.unsplash.com/photo-1567760200592-13504b6d495a?auto=format&fit=crop&w=800&q=80",
  "mirage-nightclub":
    "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&w=800&q=80",
  "pangea":
    "https://images.unsplash.com/photo-1566737236500-c8ac43014a67?auto=format&fit=crop&w=800&q=80",
  "aqwa-mist":
    "https://images.unsplash.com/photo-1543007630-9710e4a00a20?auto=format&fit=crop&w=800&q=80",
  "suite-marbella":
    "https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?auto=format&fit=crop&w=800&q=80",
  "opium-mar":
    "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?auto=format&fit=crop&w=800&q=80",

  // ── Events ────────────────────────────────────────────────────────
  "starlite-festival":
    "https://images.unsplash.com/photo-1459749411175-04bf5292ceea?auto=format&fit=crop&w=800&q=80",
  "marbella-tennis-club":
    "https://images.unsplash.com/photo-1554068865-4a89bbe24cfc?auto=format&fit=crop&w=800&q=80",
  "puente-romano-tennis":
    "https://images.unsplash.com/photo-1554068865-4a89bbe24cfc?auto=format&fit=crop&w=800&q=80",
  "yacht-club":
    "https://images.unsplash.com/photo-1567899378494-47b22a2ae96a?auto=format&fit=crop&w=800&q=80",
  "mosh-kids":
    "https://images.unsplash.com/photo-1519167758481-83f550bb49b3?auto=format&fit=crop&w=800&q=80",
  "casino-marbella":
    "https://images.unsplash.com/photo-1501281668745-b8ceab298bc1?auto=format&fit=crop&w=800&q=80",

  // ── Activités Aquatiques (Water Sports) ──────────────────────────
  "jet-ski-marbella":
    "https://images.unsplash.com/photo-1626447857058-2ba6a8868cb5?auto=format&fit=crop&w=800&q=80",
  "marbella-boat-charter":
    "https://images.unsplash.com/photo-1544551763-46a013bb70d5?auto=format&fit=crop&w=800&q=80",
  "puerto-banus-watersports":
    "https://images.unsplash.com/photo-1530870110042-98b2cb110834?auto=format&fit=crop&w=800&q=80",
  "parasailing-marbella":
    "https://images.unsplash.com/photo-1559827260-dc66d52bef19?auto=format&fit=crop&w=800&q=80",
  "flyboard-marbella":
    "https://images.unsplash.com/photo-1502933691298-84fc14542831?auto=format&fit=crop&w=800&q=80",

  // ── Hotels ────────────────────────────────────────────────────────
  "nobu-hotel-marbella":
    "https://images.unsplash.com/photo-1551882547-ff40c599fb00?auto=format&fit=crop&w=800&q=80",
  "hotel-puente-romano":
    "https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=800&q=80",
  "marbella-club-hotel":
    "https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?auto=format&fit=crop&w=800&q=80",
  "villa-padierna":
    "https://images.unsplash.com/photo-1571003123771-bd6a1e0f9e64?auto=format&fit=crop&w=800&q=80",
  "gran-melia-don-pepe":
    "https://images.unsplash.com/photo-1519672199912-a02b585df56a?auto=format&fit=crop&w=800&q=80",
  "amare-hotel-marbella":
    "https://images.unsplash.com/photo-1563911302283-d2bc129e7570?auto=format&fit=crop&w=800&q=80",
  "hotel-don-carlos":
    "https://images.unsplash.com/photo-1455587734955-081b22074882?auto=format&fit=crop&w=800&q=80",
};

// Fallback par catégorie — photos différentes des venues pour éviter tout doublon
export const CATEGORY_FALLBACK_IMAGES: Record<string, string> = {
  "Beach Club":
    "https://images.unsplash.com/photo-1540541338287-41700207dee6?auto=format&fit=crop&w=800&q=80",
  "Fine Dining":
    "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=800&q=80",
  "Spa":
    "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?auto=format&fit=crop&w=800&q=80",
  "Spa & Wellness":
    "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?auto=format&fit=crop&w=800&q=80",
  "Nightlife":
    "https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?auto=format&fit=crop&w=800&q=80",
  "Events":
    "https://images.unsplash.com/photo-1459749411175-04bf5292ceea?auto=format&fit=crop&w=800&q=80",
  "Water Sports":
    "https://images.unsplash.com/photo-1530870110042-98b2cb110834?auto=format&fit=crop&w=800&q=80",
  "Hotel":
    "https://images.unsplash.com/photo-1551882547-ff40c599fb00?auto=format&fit=crop&w=800&q=80",
};

export function getVenueImage(id: string, category?: string): string {
  return (
    VENUE_IMAGES[id] ??
    (category ? CATEGORY_FALLBACK_IMAGES[category] : null) ??
    "https://images.unsplash.com/photo-1540541338287-41700207dee6?auto=format&fit=crop&w=800&q=80"
  );
}
