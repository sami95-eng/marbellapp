...
-- -----------------------------------------------------------------
-- 2) Recalcul de profiles.partner_post_count (posts approuves)
-- -----------------------------------------------------------------
...
BEGIN
  ...
  IF uid IS NOT NULL THEN
    ...
    SET partner_post_count = (
      SELECT COUNT(*) FROM public.vip_posts vp
...
CREATE TRIGGER trg_recalc_vip_post_count
  ...
  FOR EACH ROW EXECUTE FUNCTION public.recalc_vip_post_count();

-- -----------------------------------------------------------------
-- 3) vip_tiers (config des paliers)
-- -----------------------------------------------------------------
...
CREATE POLICY "VIP tiers public read"
  ON public.vip_tiers FOR SELECT USING (TRUE);
...
FROM public.vip_tiers ORDER BY min_posts;
